...
class Room:
    ...
    async def say(self, message, success=True, type='message'):
        """
        :type message: str
        """
        ...
        for ws in self._clients:
            await ws.send(json.dumps({"text": message, "success": success, "type": type}))
...
class Server:
    def __init__(self):
        self._logger = logging.getLogger(__name__)
        self.room = Room()
        self.regexBot = re.compile('^((@|)bot |bot:)(?P<plugin>\S+)( (?P<args>.*)|)')
        self.kvsm = KVSManager()
        self.plugins = PluginLoader.get_plugins(self.kvsm)
        ...
        while True:
            ...
            if match:
                try:
                    ...
                    await self.room.say(result, type='bot')
